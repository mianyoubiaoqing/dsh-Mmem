...
The current management surface displays the exact Active Memory Space and Binding access, applies content/kind/visibility/status filters through the governed Session-bound search, lists matching governed records and Candidates, shows payload-free Observation/revision lineage, edits or explicitly merges Candidates by creating append-only replacements, supports manual and partial-success batch approval/rejection, and requires an explicit keep-both or supersede choice for blocking duplicate/conflict assessments. It also reads the versioned approval policy on demand and lets the Owner explicitly choose `manual` or `scheduled-auto` with an IANA time zone and local review time. Updates use the exact observed revision. A missing current Session fails closed, and a read-only Binding disables all writes while keeping source inspection and policy reads available.
