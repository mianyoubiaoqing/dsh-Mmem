...
This alpha is a development preview. It supports Session-bound governed record/Candidate search, filtering, payload-free provenance, append-only Candidate editing/merge, manual review, partial-success batch decisions, exact DSH Workspace-to-Memory-Space bindings, a versioned `manual`/`scheduled-auto` policy editor, and a cancellable DST-aware local scheduler with cross-process lease and daily payload-free receipts. A governance runner now requires DSH Session receipts and revalidates every trusted fact before an automatic decision. The actual rc.8 Agent Session evaluator Adapter is not bundled yet, so the scheduler remains inactive rather than accepting unlogged model output. Cross-Space sharing is also incomplete.
